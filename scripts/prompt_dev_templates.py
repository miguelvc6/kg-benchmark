...
PROMPT_DEV_TBOX_TAXONOMY_PATCH_VERSION = "prompt_dev_v5_tbox_taxonomy_patch"
...
V5_T_BOX_TAXONOMY_PATCH_CONTRACT = """Return exactly one JSON object:
{
  "case_id": "<copy input id exactly; this is the neutral prompt-visible id>",
...
def render_prompt_dev_prompt(
    *,
    ...
    if version not in {
        ...
        PROMPT_DEV_SCAFFOLDED_VERSION,
        PROMPT_DEV_DIAGNOSIS_VERSION,
    ...
    repair_version = (
        ...
        else "prompt_dev_v4_spec_only"
        if version == PROMPT_DEV_TBOX_TAXONOMY_PATCH_VERSION and task != "t_box_repair"
        else version
    )
    ...
    if task == "track_diagnosis":
        ...
        elif version == PROMPT_DEV_TBOX_TAXONOMY_PATCH_VERSION:
            task_instruction = (
                "Propose a T-box taxonomy patch for the focus property. Use only visible evidence, keep "
                "constraint-family identifiers distinct from ordinary item/type values, and report concrete value "
                "deltas only when they are visible."
            )
            contract = V5_T_BOX_TAXONOMY_PATCH_CONTRACT
        else:
            task_instruction = (
